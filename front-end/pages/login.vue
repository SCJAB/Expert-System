        ...
        <div class="bg-orange-300 bg-opacity-20 flex rounded-2xl shadow-lg max-w-3xl p-5 items-center">
            ...
            <div class="sm:w-1/2 px-8">
                ...
                <form class="flex flex-col gap-4" @submit.prevent="handleLogin()">
                  <div v-if="state.verify && state.verify.success && state.verify.success[0]" class="bg-green-500 text-white p-2 rounded-md mb-4">
...
const state = reactive({
  ...
  verify: {
    success: false // Initialize with default value
  },
})
...
async function handleLogin(){
  ...
  try{
    const response = await $fetch(`http://127.0.0.1:8000/api/login-takers`, {
    method: 'POST',
    body: params
  })
